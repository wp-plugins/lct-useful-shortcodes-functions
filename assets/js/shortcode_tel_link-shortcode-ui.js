...
var LCTSCtellinkShortcodeUI;

( function (lctShortCodeUI, $) {

    var sui = window.LCTSCtellinkShortcodeUI = {
        models: {},
        collections: {},
        views: {},
        utils: {},
    ...
    sui.views.editShortcodeForm = wp.Backbone.View.extend({
        ...
        el: '#LCT_SC_tel_link-shortcode-ui-container',

        template: wp.template('lct-shortcode-default-edit-form'),

        hasAdvancedValue: false,

        events: {
            'click #LCT_SC_tel_link-update-shortcode': 'insertShortcode',
            'click #LCT_SC_tel_link-insert-shortcode': 'insertShortcode',
            'click #LCT_SC_tel_link-cancel-shortcode': 'cancelShortcode'
        },
        ...
        afterRender: function () {
            ...
            $('#LCT_SC_tel_link-insert-shortcode').toggle(this.options.viewMode == 'insert');
            $('#LCT_SC_tel_link-update-shortcode').toggle(this.options.viewMode != 'insert');
        ...
        dispose: function () {
            this.remove();
            $('#LCT_SC_tel_link-shortcode-ui-wrap').append('<div id="LCT_SC_tel_link-shortcode-ui-container"></div>');
        }
    });
    ...
    sui.views.editAttributeField = Backbone.View.extend({
        ...
        updateAction: function (e) {
            ...
            LCTSCtellinkShortcodeUI = new sui.views.editShortcodeForm({model: newShortcodeModel, viewMode: viewMode});
            LCTSCtellinkShortcodeUI.render();
    ...
    sui.utils.shortcodeViewConstructor = {
        ...
        fetch : function() {
            ...
            if ( ! this.fetching ) {
                ...
                data = {
                    ...
                    nonce: lctShortcodeUIData.previewNonce
                };

                $.post(ajaxurl, data).done(function(response) {
                    self.content = response;
                }).fail(function () {
                    self.content = '<span class="lct_shortcode_ui_error">' + lctShortcodeUIData.strings.errorLoadingPreview + '</span>';
                }).always(function () {
                ...
                this.getNodes(function (editor, node, content) {
                    ...
                    setTimeout(function () {
                        iframe = dom.add(content, 'iframe', {
                            ...
                            frameBorder: '0',
                            id: 'LCT_SC_tel_link-shortcode-preview-' + new Date().getTime(),
        ...
        edit : function( shortcodeString ) {
            ...
            if ( currentShortcode ) {
                ...
                $('#add_LCT_SC_tel_link_id').val(formId);

                LCTSCtellinkShortcodeUI = new sui.views.editShortcodeForm({model: currentShortcodeModel, viewMode: 'update'});
                LCTSCtellinkShortcodeUI.render();

                $('#LCT_SC_tel_link-insert-shortcode').hide();
                $('#LCT_SC_tel_link-update-shortcode').show();
                tb_show("Edit Tel Link", "#TB_inline?inlineId=select_LCT_SC_tel_link&width=753&height=686", "");
    ...
    $(document).ready(function () {
        ...
        sui.strings = lctShortcodeUIData.strings;

        sui.shortcodes = new sui.collections.Shortcodes( lctShortcodeUIData.shortcodes );

        if( ! lctShortcodeUIData.previewDisabled && typeof wp.mce != 'undefined'){
            wp.mce.views.register( 'lct_tel_link', $.extend(true, {}, sui.utils.shortcodeViewConstructor) );
        }

        $(document).on('click', '.LCT_SC_tel_link_media_link', function () {
            sui.shortcodes = new sui.collections.Shortcodes(lctShortcodeUIData.shortcodes);
            var shortcode = sui.shortcodes.findWhere({shortcode_tag: 'lct_tel_link', action_tag: ''});
            LCTSCtellinkShortcodeUI = new sui.views.editShortcodeForm({model: shortcode, viewMode: 'insert'});
            LCTSCtellinkShortcodeUI.render();
            tb_show("Insert a Tel Link", "#TB_inline?inlineId=select_LCT_SC_tel_link&width=753&height=686", "");
        });

    });

}(window.lctShortcodeUI = window.lctShortcodeUI || {}, jQuery));
